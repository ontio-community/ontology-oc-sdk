...
static MnemonicCode *_instance = nil;
/**
 * @brief bip39_english wordlist
 */
+(instancetype)shareInstance{
    static dispatch_once_t onceToken ;
    dispatch_once(&onceToken, ^{
        NSMutableArray *array = [NSMutableArray arrayWithCapacity:BIP39_PBKDF2_ROUNDS];
        for (int i = 0; i < BIP39_PBKDF2_ROUNDS; i++) {
            [array addObject:[NSString stringWithUTF8String:wordlist[i]]];
        }
        _instance = [[super allocWithZone:NULL] initWithWordList:array];
    }) ;
    
    return _instance ;
}
+(id) allocWithZone:(struct _NSZone *)zone{
    return [MnemonicCode shareInstance] ;
}
